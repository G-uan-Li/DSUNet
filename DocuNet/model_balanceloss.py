...
class DocREModel(nn.Module):
    ...
    def __init__(self, config, args, model, block_size=64, num_labels=-1):
        super().__init__()
        ...
        self.head_extractor = nn.Linear(
            1 * config.hidden_size + args.unet_out_dim, config.hidden_size
        )
        self.tail_extractor = nn.Linear(
            1 * config.hidden_size + args.unet_out_dim, config.hidden_size
        )

        self.enhanced_head_extractor = nn.Linear(
            2 * config.hidden_size, config.hidden_size
        )
        self.enhanced_tail_extractor = nn.Linear(
            2 * config.hidden_size, config.hidden_size
        )
    ...
    def forward(
        ...
        if self.channel_type == "context-based":
            ...
            # print("attn_input.shape: ", attn_input.shape)  # torch.Size([4, 3, 35, 35])
        ...
        # print("output.shape: {}".format(output.shape))  # torch.Size([M, 97])
        if labels is not None:
            labels = [torch.tensor(label) for label in labels]
            labels = torch.cat(labels, dim=0).to(logits)
            # print("labels.shape: ", labels.shape)  # torch.Size([M, 97])
            # print("labels ", labels)
