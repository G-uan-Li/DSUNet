...
class AttentionUNet(torch.nn.Module):
    ...
    def forward(self, attention_channels):
        ...
        # print("UNet input shape: ", x.shape)  # torch.Size([4, 3, 42, 42])
        x1 = self.inc(x)
        # print("after InConv, x1.shape: ", x1.shape)  # torch.Size([4, 256, 42, 42])
        x2 = self.down1(x1)
        # print("x2.shape: ", x2.shape)  # torch.Size([4, 512, 21, 21])
        x3 = self.down2(x2)
        # print("x3.shape: ", x3.shape)  # torch.Size([4, 512, 10, 10])
        x = self.up1(x3, x2)
        # print("x.shape: ", x.shape)  # torch.Size([4, 256, 21, 21])
        x = self.up2(x, x1)
        # print("x.shape: ", x.shape)  # torch.Size([4, 128, 42, 42])
        output = self.outc(x)
        # print("output.shape: ", output.shape)  # torch.Size([4, 256, 42, 42])
        # attn_map as the shape of: batch_size x width x height x class
        output = output.permute(0, 2, 3, 1).contiguous()
        # print("output.shape: ", output.shape)  # torch.Size([4, 42, 42, 256]) 对应x1, [4, 256, 42, 42]
        return output
