...
class ReadDataset:
    def __init__(
        ...
        transformers: str = "bert",  # 如果tokenizer是bert，那么transformers就是bert
    ):
...
def read_docred(transfermers, file_in, save_file, tokenizer, max_seq_length=1024):
    ...
    with open(file_in, "r") as fh:
        data = json.load(fh)
    if transfermers == "bert":  # 如果预训练模型是roberta呢? transformers 也设置为 "bert"?
    ...
    """
    docred 格式，每篇文档对应一个 dict，包含以下字段：
    {
        "vertexSet": ,
        "labels": ,
        "title": "Washington Place (West Virginia)",
        "sents": 
    },
    ...
    for sample in tqdm(data, desc="Example"):
        ...
        entities = sample["vertexSet"]  # list
        entity_start, entity_end = [], []
        mention_types = []
        for entity in entities:  # list
            for mention in entity:  # dict
                """
                mention的格式:
                {
                    "name": "Greek",
        ...
        for i_s, sent in enumerate(sample["sents"]):
            new_map = {}
            for i_t, token in enumerate(sent):  # token is word now
                tokens_wordpiece = tokenizer.tokenize(token)
                if (i_s, i_t) in entity_start:
                    t = entity_start.index((i_s, i_t))
                    if (
                        transfermers == "bert"
                    ):  # 是不是用 roberta，也要将 transformers 设置为 "bert"?
        ...
        feature = {
            "input_ids": input_ids,
            "entity_pos": entity_pos,  # 实体在文档中的位置
            "labels": relations,
            "hts": hts,  # head tail ?  [relation, evidence, label]
            "title": sample["title"],
        }
        features.append(feature)  # 对应一篇文档 ?
    ...
    # print(
    #     "finish reading {} and save preprocessed data to {}.".format(file_in, save_file)
    # )
...
def read_gda(file_in, save_file, tokenizer, max_seq_length=1024):
    if os.path.exists(save_file):
        ...
        with open(file=save_file, mode="wb") as fw:
            pickle.dump(features, fw)
        print(
            "finish reading {} and save preprocessed data to {}.".format(
                file_in, save_file
            )
        )
